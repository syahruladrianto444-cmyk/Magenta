...
@section('content')
    <div class="mb-8">
        <div class="flex items-center text-gray-500 dark:text-gray-400 mb-2">
            <a href="{{ route('admin.events.index') }}" class="hover:text-primary-500 transition-colors">Event
                Management</a>
            <i data-lucide="chevron-right" class="w-4 h-4 mx-2"></i>
            <span class="text-gray-900 dark:text-white">Edit Event</span>
        </div>
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Edit Event</h1>
    </div>

    <div
        class="bg-white dark:bg-navy-800 rounded-2xl border border-gray-200 dark:border-navy-700 shadow-sm dark:shadow-none p-8">
        <form action="{{ route('admin.events.update', $event) }}" method="POST"
            x-data="{ videoId: '{{ old('youtube_video_id', $event->youtube_video_id ?? '') }}', showPreview: {{ $event->youtube_video_id ? 'true' : 'false' }} }"
            ...
            <div class="grid md:grid-cols-2 gap-6">
                {{-- Event Name --}}
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Nama Event <span
                ...
                {{-- Status --}}
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Status <span
                            class="text-red-500">*</span></label>
                    <select name="status"
                        class="w-full px-4 py-3 bg-gray-50 dark:bg-navy-900 border border-gray-300 dark:border-navy-700 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-colors">
                        <option value="preparing" {{ old('status', $event->status) === 'preparing' ? 'selected' : '' }}>🔧
                            Persiapan</option>
                        <option value="live" {{ old('status', $event->status) === 'live' ? 'selected' : '' }}>🔴 Live</option>
                ...
                {{-- PIC / Client --}}
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Assign Client /
                        PIC</label>
                    <select name="client_ids[]" multiple
                        class="w-full px-4 py-3 bg-gray-50 dark:bg-navy-900 border border-gray-300 dark:border-navy-700 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-colors"
                        style="min-height: 48px;">
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ in_array($client->id, old('client_ids', $assignedClientIds)) ? 'selected' : '' }}>
                                {{ $client->name }} ({{ $client->email }})
                            </option>
                        @endforeach
                    </select>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Tahan Ctrl/Cmd untuk memilih lebih dari satu
                    </p>
            ...
            <div class="mt-6">
                ...
                <input type="text" name="youtube_video_id" value="{{ old('youtube_video_id', $event->youtube_video_id) }}"
                    x-model="videoId"
                    class="w-full px-4 py-3 bg-gray-50 dark:bg-navy-900 border border-gray-300 dark:border-navy-700 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-colors"
                    placeholder="Contoh: SK_7toXT1Bc">
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Masukkan ID video dari URL YouTube (bagian setelah
                    v=)</p>
                ...
                <div x-show="showPreview" x-transition class="mt-4">
                    ...
                    <div class="relative bg-black rounded-xl overflow-hidden shadow-lg max-w-lg"
